...
const els = {
  ...
  discordLogin: document.querySelector("[data-discord-login]"),
  ...
  profileAvatar: document.querySelector("[data-profile-chip] img"),
...
async function syncRemoteSessionFromAnotherTab(reason = "auth-sync") {
  ...
  try {
    ...
    if (hasNewLogin || justConfirmed || reason === "email-confirmed") {
      ...
      setStatus("Account collegato: ricarico la pagina gia aperta...", "success");
...
async function applyAuthSync(payload) {
  ...
  setStatus("Account collegato: aggiorno la pagina gia aperta...", "success");
    ...
    ? users.map((item) => {
      ...
      return `<article class="ceo-user-card">
        ...
        <dl>
          ...
          <dt>Account verificato</dt><dd>${item.emailConfirmedAt || provider === "discord" ? "Si" : "No"}</dd>
...
function authProviders(authUser) {
  ...
  const fromApp = Array.isArray(app.providers) ? app.providers : [app.provider].filter(Boolean);
  const fromIdentities = Array.isArray(authUser?.identities)
    ? authUser.identities.map((identity) => identity.provider).filter(Boolean)
    : [];
  return [...new Set([...fromApp, ...fromIdentities].map((provider) => String(provider).toLowerCase()))];
}

function isDiscordAuthUser(authUser) {
  return authProviders(authUser).includes("discord");
}
...
async function completeEmailRedirectLogin() {
  ...
  if (code) {
    ...
    setStatus("Account collegato: login effettuato automaticamente.", "success");
...
async function syncRemoteProfileSeen(token) {
  const base = ceoApiBase();
  if (!base || !token) return null;
  const response = await fetch(`${base}/profile/seen`, {
    method: "POST",
    headers: {
...
async function loadRemoteSession() {
  ...
  const syncedProfile = await syncRemoteProfileSeen(session.access_token).catch(() => null);

  let profile = syncedProfile;
  if (!profile) {
    const { data: selectedProfile, error: profileError } = await supabaseClient
      .from("profiles")
      .select("id,nickname,nickname_key,role,nickname_changed_at,provider,discord_id,discord_username,discord_avatar_url,discord_email")
  ...
  if (!profile) {
    remoteUser = null;
    state.sessionNickname = "";
    setStatus("Account creato, ma profilo non pronto: riprova tra pochi secondi.", "error");
    return;
  }
...
async function recordRemoteProfileSeen() {
  ...
  if (!token) return;
  await syncRemoteProfileSeen(token).catch(() => {});
}
...
async function handleDiscordLogin() {
  if (!supabaseClient) {
    return setStatus("Login Discord disponibile solo sul sito online.", "error");
  }
  setStatus("Ti porto su Discord per il login...", "");
  try {
    sessionStorage.setItem(DISCORD_OAUTH_KEY, "1");
  } catch {}
  ...
  if (error) {
    ...
    setStatus(error.message || "Login Discord non riuscito.", "error");
  }
}
...
els.discordLogin?.addEventListener("click", handleDiscordLogin);
...
els.downloadLink.addEventListener("click", async (event) => {
  ...
  if (!user) {
    window.location.hash = "commenti";
    setDownloadStatus("Per scaricare devi accedere con Discord.", "error");
    return setStatus("Per scaricare devi accedere con Discord.", "error");
  }
  if (isRemoteMode() && !user.canDownload) {
    window.location.hash = "commenti";
    setDownloadStatus("Account non verificato: rifai il login Discord e riprova.", "error");
    return setStatus("Account non verificato: rifai il login Discord e riprova.", "error");
  }
  if (isRemoteMode()) {
    try {
